**ΠΕΡΙΠΤΩΣΗ ΧΡΗΣΗΣ : ΠΑΡΑΔΟΣΗ ΠΑΡΑΓΓΕΛΙΑΣ / ΟΛΟΚΛΗΡΩΣΗ ΤΗΣ ΔΙΑΔΙΚΑΣΙΑΣ**

**Πρωτεύων Actor** : Διανομέας  
**Ενδιαφερόμενοι** :   
 Διαχειριστής : Επιθυμεί την ομαλή παράδοση της παραγγελίας  
 Διανομέας : Είναι υπεύθυνος για την παράδοση της παραγγελίας  
 Πελάτης : Θέλει να παραλάβει την παραγγελία που ζήτησε  
 **Προϋποθέσεις**: 
 1) Ο διανομέας έχει ταυτοποιηθεί από το σύστημα
 2) Η παραγγελία υπάρχει στο σύστημα από τα προηγούμενα στάδια 

 **Βασική Ροή** :  
 1) Ο διανομέας αναζητάει την παραγγελία στη λιστα με τις παραγγελιες του  
 2) Αφού την βρει ενημερώνει την κατάστασή της  

   Διάγραμμα δραστηριοτήτων  
![Διάγραμμα δραστηριοτήτων](../docs/markdown/uml/Activity_Diagrams/delivery.png)  

   Διάγραμμα ακολουθίας
![Διάγραμμα ακολουθίας](../docs/markdown/uml/Sequence_Diagrams/delivery_seq.png)

